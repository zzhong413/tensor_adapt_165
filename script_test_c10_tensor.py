...
dataroot = '--dataroot '
dataroot += 'datasets/'		# PLEASE EDIT THIS
...
for corruption in common_corruptions:
	...
	call(' '.join(['python', 'test_calls/test_initial.py',
						...
						'--resume 		results/cifar10_tensor_%s_%s/' %(shared, name),
						'--outf 		results/C10C_tensor_%s_%s_%s%s/' %(shared, setting, name, fix_str)]),
						shell=True)
	...
	call(' '.join(['python', 'test_calls/test_adapt_tensor.py',
						dataroot,
						gpnorm_tag,
						online_tag,
						fix_tag,
