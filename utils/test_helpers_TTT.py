...
def build_model_modules(args):
    if args.tensor:
        from models.ResNet_tensor_adapt_TTT import ResNetCifar as ResNet
    else:
        from models.ResNet import ResNetCifar as ResNet

    if args.tensor:
        from models.SSHead_adapt_TTT import ExtractorHead
    else:
    ...
    if args.shared == 'layer2':
        if args.tensor == True:
            from models.SSHead_adapt_TTT import extractor_from_layer2, Head_on_layer2
            ext = extractor_from_layer2(net)
            head = Head_on_layer2(width=args.width, classes=4, depth=args.depth)
        else:
            from models.SSHead import extractor_from_layer2, head_on_layer2
            ext = extractor_from_layer2(net)
            head = head_on_layer2(net=net, width=args.width, classes=4)
...
def test(dataloader, model, sslabel=None, adapt=False):
    ...
    correct = []
    losses = []
    for batch_idx, (inputs, labels, data_idx) in enumerate(dataloader):
